...
{
    public partial class PrepareForTenantMoveQueue : Page
    {
        #region methods

        protected void Filter_SelectedIndexChanged(object sender, EventArgs e)
        {
            Response.Redirect(string.Format("~/Queues/PrepareForTenantMoveQueue.aspx?Status={1}",
                Request.QueryString["id"], Filter.SelectedValue));
        }

        protected void Page_Load(object sender, EventArgs e)
        {
            Title = "Prepare for Tenant Move";
        ...
        private void LoadList()
        {
            ...
            queueList.DataSource = TableGroupActionQueue.GetQueuedRequestsByStatus<PrepareForTenantMoveRequest>(filter);
            queueList.DataBind();
        }
