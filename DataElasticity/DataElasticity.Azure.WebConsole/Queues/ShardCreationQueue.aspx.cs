...
{
    public partial class ShardCreationQueue : Page
    {
        #region methods

        protected void Filter_SelectedIndexChanged(object sender, EventArgs e)
        {
            Response.Redirect(string.Format("~/Queues/ShardCreationQueue.aspx?Status={1}", Request.QueryString["id"],
                Filter.SelectedValue));
        }

        protected void Page_Load(object sender, EventArgs e)
        {
            Title = "Shard Creation Queue";
        ...
        private void LoadList()
        {
            ...
            queueList.DataSource = TableGroupActionQueue.GetQueuedRequestsByStatus<ShardCreationRequest>(filter);
            queueList.DataBind();
        }
