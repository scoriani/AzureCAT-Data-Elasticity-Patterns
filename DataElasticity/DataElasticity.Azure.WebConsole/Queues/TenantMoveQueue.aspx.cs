...
{
    public partial class TenantMoveQueue : Page
    {
        #region methods

        protected void Filter_SelectedIndexChanged(object sender, EventArgs e)
        {
            Response.Redirect(string.Format("~/Queues/TenantMoveQueue.aspx?Status={1}", Request.QueryString["id"],
                Filter.SelectedValue));
        }

        protected void Page_Load(object sender, EventArgs e)
        {
            Title = "Tenant Move Queue";
        ...
        private void LoadList()
        {
            ...
            queueList.DataSource = TableGroupActionQueue.GetQueuedRequestsByStatus<TenantMoveRequest>(filter);
            queueList.DataBind();
        }
